...
describe("hosted-fields-component", () => {
  ...
  beforeEach(() => {
    ...
    td.when(
      fakeHostedFieldsInstance.tokenize(td.matchers.isA(Object))
    ).thenResolve(fakeTokenizationPayload);
    ...
    td.when(hostedFieldsCreate(td.matchers.isA(Object))).thenResolve(
  ...
  it("rejects if no payments function is provided", () => {
    ...
    return HostedFields.render(renderOptions, "#button")
  ...
  it("resolves with an object that can tokenize", () => {
    return HostedFields.render(renderOptions, "#button")
      ...
      .then(() => {
        td.verify(
          fakeHostedFieldsInstance.tokenize({
            vault: false,
            orderId: "order-id",
          })
        );
  ...
  it("rejects render with an error if button element cannot be found", () => {
    return HostedFields.render(renderOptions, "#button2")
      .then(rejectIfResolves)
      .catch((err) => {
        // $FlowFixMe
        assert.equal(
          err.message,
          "Could not find selector `#button2` on the page"
        );
      });
  });
  ...
  it("calls onApprove function with tokenization data if passed in", (done) => {
    ...
    HostedFields.render(renderOptions, "#button2")
      .then((handler) => {
  ...
  describe("#submit", () => {
    ...
    it("runs through contingency flow if error is 3ds contingency and resolves if contingency resolves", () => {
      const expectedUrl =
        "https://www.paypal.com/webapps/helios?action=resolve&flow=3ds&cart_id=21E005655U660730L";
      const error = {
        ...
        details: [
          {
            issue: "CONTINGENCY",
            description:
              "Buyer needs to resolve following contingency before proceeding with payment",
          },
        ],
        links: [
          {
      ...
      td.when(
        contingencyFlowStart(getContingencyFlowComponent(), expectedUrl)
      ).thenResolve(threeDSResult);
    ...
    it("resolve 3ds contingency and return liabilityShifted true if liability_shifted is YES", () => {
      ...
      return HostedFields.render(renderOptions, "#button").then((handler) => {
        return handler.submit().then((actual) => {
          ...
          assert.equal(
            actual.authenticationReason,
            threeDSResult.authentication_status_reason
          );
        });
      });
    });
