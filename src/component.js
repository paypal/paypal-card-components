  ...
  return (options = {}) => {
    ...
    if (paymentInProgress) {
      return ZalgoPromise.reject(
        new Error("Hosted Fields payment is already in progress.")
      );
    }
    ...
    return orderIdFunction().then((orderId) => {
      ...
      logger.track({
        ...
        payments_schedule: options.hasOwnProperty("installments")
        ...
        .catch((err) => {
          if (
            !(
              err.details &&
              err.details.find &&
              err.details.find((detail) => detail.issue === "CONTINGENCY")
            )
          ) {
          ...
          const url = `${
            err.links.find((link) => link.rel === "3ds-contingency-resolution")
              .href
          }`;
        ...
        .then((payload) => {
          ...
          logger.track({
            ...
            card_brand:
              ...
              payload.payment_source.card.card_type,
          ...
          return {
            ...
            card:
              payload && payload.payment_source && payload.payment_source.card,
            orderId,
          };
        })
        .catch((err) => {
...
export const HostedFields = {
  isEligible(): boolean {
    ...
    if (fundingEligibility && fundingEligibility.card) {
      return Boolean(
        fundingEligibility.card.eligible && !fundingEligibility.card.branded
      );
    }
  ...
  render(
  ...
  ): ZalgoPromise<HostedFieldsHandler> {
    ...
    if (typeof options.createOrder !== "function") {
      return ZalgoPromise.reject(
        new Error("createOrder parameter must be a function.")
      );
    }
    ...
    if (options.installments) {
      if (
        ...
        return ZalgoPromise.reject(
          new Error(
            "installments must include both onInstallmentsRequested and onInstallmentsAvailable functions"
          )
        );
      }
    ...
    return getUccEligibility.then((eligibilityData) => {
      if (
        !eligibilityData ||
        !eligibilityData.card ||
        !eligibilityData.card.eligible ||
        eligibilityData.card.branded
      ) {
        logger.warn("HOSTEDFIELDS_NOT_ELIGIBLE");
        // inEligible
        return ZalgoPromise.reject(
          new Error("hosted fields are not eligible.")
        );
      }
      ...
      const cardVendors =
        (configuration.fundingEligibility &&
          configuration.fundingEligibility.card &&
          configuration.fundingEligibility.card.vendors) ||
        {};
      const eligibleCards = Object.keys(cardVendors).filter(
        (key) => cardVendors[key] && cardVendors[key].eligible
      );
      ...
      if (buttonSelector && options.onApprove) {
        ...
        if (!button) {
          return ZalgoPromise.reject(
            new Error(
              `Could not find selector \`${buttonSelector}\` on the page`
            )
          );
        }
      }
      ...
      return btClient
        ...
        .then((hostedFieldsInstance) => {
          hostedFieldsInstance.submit = createSubmitHandler(
          ...
          logger.track({
            ...
            hosted_payment_textboxes_shown: Object.keys(
              hostedFieldsCreateOptions.fields
            ).join(":"),
            hosted_payment_session_cre_dt: new Date().toString(),
            hosted_payment_session_cre_ts_epoch: Date.now().toString(),
